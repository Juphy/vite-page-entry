            ...
            <div class="form-area" v-if="showFormArea">
                ...
                >
                    <el-form-item label="房间ID" prop="roomId">
                        <el-input
                            v-model.trim="roomForm.roomId"
                            :disabled="!canClickBtn"
                        ></el-input>
                    </el-form-item>
                    <el-form-item label="姓名" prop="userName">
                        <el-input
                            v-model.trim="roomForm.userName"
                            :disabled="!canClickBtn"
                        ></el-input>
                    </el-form-item>
                    <el-form-item>
...
export default {
    ...
    data() {
        const validateRoomId = (rule, value, callback) => {
            const reg = /^\d{1,4}$/
            if (!reg.test(value)) {
                return callback(new Error("房间ID只能为1-4位数字"))
            }
            callback()
        }
    ...
    methods: {
        canSupportWebRTC() {
            if (typeof navigator.mediaDevices !== "object") {
                ElMessage({
                    message: "No navigator.mediaDevices",
                    type: "error",
                })
                return false
            }
            if (typeof navigator.mediaDevices.enumerateDevices !== "function") {
                ElMessage({
                    message: "No navigator.mediaDevices.enumerateDevices",
                    type: "error",
                })
                return false
            }
            if (typeof navigator.mediaDevices.getUserMedia !== "function") {
                ElMessage({
                    message: "No navigator.mediaDevices.getUserMedia",
                    type: "error",
                })
                return false
            }
        ...
        initSocketEvents() {
            ...
            socket.on("checkRoomSuccess", (exsitRoomUsers) => {
                ...
                if (exsitRoomUsers && exsitRoomUsers.length > 3) {
                    ElMessage({ message: "当前房间人数已满~请换个房间id" })
                } else {
            ...
            socket.on("joinRoomSuccess", (roomUsers) => {
                console.log("joinRoomSuccess client user:", roomUsers, this.sockId)
                const otherUser = roomUsers.find(
                    (item) => item.sockId !== this.sockId
                )
                if (!otherUser) return false
                ElMessage({ message: `${otherUser.userName}加入了房间`, type: "success" })
                this.roomUsers = [
                    otherUser,
                    {
                        userName: this.roomForm.userName + "(我)",
            ...
            socket.on("userLeave", (roomUsers) => {
                ...
                this.roomUsers.forEach((item) => {
                    if (serverSockIdArr.indexOf(item.sockId) === -1) {
                        ElMessage({ message: `${item.userName}离开了房间` })                        
            ...
            socket.on("answerVideo", async (user) => {
                ...
                document.querySelector("#echat-local").srcObject =
                    this.localStream
        ...
        submitForm() {
            if (!this.sockId) {
                ElMessage({ message: "socket未连接成功,请刷新再尝试!", type: "error" })                
        ...
        initPeerListen() {
            ...
            this.peer.onaddstream = (event) => {
                // 拿到对方的视频流
                document.querySelector("#echat-remote-1").srcObject =
                    event.stream
            }
            this.peer.onclose = () => {}
        },
